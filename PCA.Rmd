...
Model (self-efficacy)
```{r}
priors.slf <- c(prior(normal(0, 10), "b"), prior(normal(0, 50), "intercept"),     prior(student_t(3, 0, 20), "sd"), prior(student_t(3, 0, 20), "sigma"))

model_slf <- brm(consumption ~ self_efficacy + fish_availability + preparation, data = full_data3, family = gaussian(), iter = 10000, save_pars(all), prior = priors.slf, control = list(adapt_delta = 0.99))
```
