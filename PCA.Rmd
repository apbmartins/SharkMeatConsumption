...
Setting flat priors
```{r}
bprior <- prior(normal(0,10), class = b, coef = mislabelling)
```

Model (mislabelling)
```{r}
model_mis <- brm(consumption ~ mislabelling, data = full_data4, family = gaussian(), iter = 10000, save_pars = save_pars(all = TRUE), prior = bprior, control = list(adapt_delta = 0.99))
summary(model_mis)
plot(model_mis)
```
...
Model (self_efficacy)
```{r}
cprior <- c(prior(normal(0,10), class = b, coef = fish_availability), prior(normal(0,10), class = b, coef = preparation), prior(normal(0,10), class = b, coef = self_efficacy))

model_sel <- brm(consumption ~ self_efficacy + fish_availability + preparation, data = full_data4, family = gaussian(), iter = 10000, save_pars = save_pars(all = TRUE), prior = cprior, control = list(adapt_delta = 0.99))

summary(model_sel)
plot(model_sel)
```

Model (culinary_tradition)
```{r}
dprior <- c(prior(normal(0,10), class = b, coef = culinary_traditions), prior(normal(0,10), class = b, coef = religious_traditions))

model_cul <- brm(consumption ~ culinary_traditions + religious_traditions, data = full_data4, family = gaussian(), iter = 10000, save_pars = save_pars(all = TRUE), prior = dprior, control = list(adapt_delta = 0.99))

summary(model_cul)
plot(model_cul)
```

Model (health_beliefs)
```{r}
eprior <- c(prior(normal(0,10), class = b, coef = health_beliefs), prior(normal(0,10), class = b, coef = marketing))
...
Model (eating_habits)
```{r}
fprior <- c(prior(normal(0,10), class = b, coef = culinary_traditions), prior(normal(0,10), class = b, coef = marketing), prior(normal(0,10), class = b, coef = eating_habits))
